        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                for (var i = 0; i < files.length; i++) {
                    ...
                    data.append("ProdukID", $("#ComboCampaignCollection").val());

                    request = $.ajax({
                        type: "POST",
                        enctype: 'multipart/form-data',
                        url: "asmx/Coll_TrxUpload.asmx/CollUpload_1",
                        data: data,
                        // dataType: "json",
                        contentType: false,
                        processData: false,
                    ...
                    request.done(function (response) {
                        ...
                        swal(
                            '',
                            'Upload Data Collection Has Been Success',
                            'success'
                        ).then(function () {
                            $("#ButtonDistribute").show();
                            $("#ButtonDelete").show();
                            location.href = "Coll_TrxUpload.aspx?id=" + $("#ContentPlaceHolder1_TrxID").val() + "";
                        });
                        // result = response.d;
                    ...
                    request.fail(function (response) {
                        ...
                        swal(
                            '',
                            'Upload Data Collection Has Been Failed',
                            'error'
                        ).then(function () {
                            return false;
                        });
...
function CmbProductColl(UploadID) {
    var ComboCampaign = $('#ComboCampaignCollection');
    $.ajax({
        type: "POST",
        url: "WebServiceGetDataMaster.asmx/UIDESK_TrmMasterCombo",
        data: "{TrxID:'UideskIndonesia', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK93'}",
...
function OnchangeComboUpload() {
    location.href = "Coll_TrxUpload.aspx?p_re=1&id=" + $("#ComboUploadID").val() + "";
}
    ...
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                $.ajax({
                    url: "asmx/Coll_TrxUpload.asmx/DistributeData",
                    method: "POST",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    data: form_data,
                        ...
                        for (i = 0; i < json.length; i++) {
                            ...
                            if (json[i].Result == "True") {
                                swal(
                                    ...
                                    location.href = "Coll_TrxUpload.aspx?";
    ...
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                $.ajax({
                    url: "asmx/Coll_TrxUpload.asmx/DistributeDelete",
                    method: "POST",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    data: form_data,
                        ...
                        for (i = 0; i < json.length; i++) {
                            ...
                            if (json[i].Result == "True") {
                                swal(
                                    ...
                                    location.href = "Coll_TrxUpload.aspx?p_re=1&id=" + getParameterByName("id") + "";
                                });
                            } else {
                                swal(
                                    '',
...
function AgentLogin() {
    ...
    $.ajax({
        type: "POST",
        url: "WebServiceGetDataMaster.asmx/UIDESK_TrmMasterCombo",
        data: "{TrxID:'defaultSystem', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK105'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            var json = JSON.parse(data.d);
                ...
                for (i = 0; i < json.length; i++) {
                    ...
                    if (json[i].Login == '1') {
                        if (json[i].AuxID == '9') {
                            ...
                            var statusAgent = "<i class='fa fa-circle-o text-warning'></i>"
                        }
                    } else {
...
function searchingAgentLogin(sValue) {
    ...
    $.ajax({
        type: "POST",
        url: "WebServiceGetDataMaster.asmx/UIDESK_TrmMasterCombo",
        data: "{TrxID:'" + selectedValue + "', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK105'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            var json = JSON.parse(data.d);
