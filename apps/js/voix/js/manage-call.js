...
var audioOne = document.querySelector('#audioIncoming');
...
function windowsNotifIncomingCall(fromNo) {
    if (Notification.permission === "granted") {
        const notification = new Notification("You have a call from : " + fromNo, {
            body: "Hi there. Please answer this call !",
            icon: "phone-call.png"
        })
        stopAllAudio();
        //var source = "audio/iphone.mp3";
        //var audio = new Audio(); // use the constructor in JavaScript, just easier that way
        audioOne.addEventListener("load", function () {
            audio.play();
        }, true);
       
        audioOne.autoplay = true; // add this
...
const handleIncomingCall = ({ call }) => {
    currentCall = call;
    console.log("ada call masuk");
    audioOne.addEventListener("load", function () {
        audio.play();
    }, true);

    audioOne.autoplay = true; // add this
    windowsNotifIncomingCall(call.number())
    document.getElementById('menuCallIncoming').classList.add('control-sidebar-open');
  cleanData();
...
const setUpCall = ({ currentCall, isIncoming, number, viewer, video }) => {
  ...
  currentCall.addEventListener(VoxImplant.CallEvents.Disconnected, () => {
    stopTimer();
      ...
      console.log("ada call selesai");
      document.getElementById('menuCallIncoming').classList.remove('control-sidebar-open');
    callStateDisconnected();
  });
