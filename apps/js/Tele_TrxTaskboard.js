            ...
            if (json.length == 0) {
                ...
                for (i = 0; i < json.length; i++) {
                    ...
                    var urlClick = "<div class='dropdown'>" +
                        ...
                        "<a class='dropdown-item' href='Tele_TrxTransaksi.aspx?id=" + json[i].TrxID + "&name=" + json[i].TrxName + "&ph=" + json[i].TrxTelepon + "&mail=" + json[i].TrxEmail + "&script=" + json[i].TrxProdukID +"'><i class='si-arrow-right-circle si'></i> Follow Up</a>" +
                        "</div>" +
                        "</div>"

                    if (json[i].TrxCounting == null || json[i].TrxCounting == "") {
                        var CallCounting = "<span class='badge badge-pill badge-danger'>0</span>"
                    } else {
                        var CallCounting = "<span class='badge badge-pill badge-danger'>" + json[i].TrxCounting + "</span>"
                    }
                    myTable.row.add([json[i].TrxID, json[i].TrxName, json[i].TrxEmail, json[i].TrxTelepon, json[i].TrxStatus, json[i].TrxAddress, newDate + ' ' + newTime, CallCounting, urlClick]).draw(false);
...
function DataTablePending(sValue) {
    var result = "";
    var myTable = $('#TrmTeleHeaderPending').DataTable();
    $.ajax({
        type: "POST",
        url: "asmx/Tele_TrxTaskboard.asmx/TeleTaskboardPending",
            ...
            if (json.length == 0) {
                ...
                for (i = 0; i < json.length; i++) {
                    ...
                    var urlClick = "<div class='dropdown'>" +
                        ...
                        "<div class='dropdown-divider'></div>" +
                        "<a class='dropdown-item' href='Tele_TrxTransaksi.aspx?id=" + json[i].ID + "&name=" + json[i].Name + "&ph=" + json[i].Telepon + "&mail=" + json[i].Email + "&script=" + json[i].ProdukID + "'><i class='si-arrow-right-circle si'></i> Follow Up</a>" +
                        "</div>" +
                        "</div>"
