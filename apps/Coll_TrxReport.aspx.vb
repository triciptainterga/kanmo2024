...
Public Class Coll_TrxReport
    ...
    Private Sub btn_Submit_Click(sender As Object, e As EventArgs) Handles btn_Submit.Click
        Dim queryInsert As String = "exec Coll_XtraReport_Transaksi '" & Session("UserName") & "','" & Format(dt_strdate.Value, "yyyy-MM-dd") & "','" & Format(dt_endate.Value, "yyyy-MM-dd") & "'"
        ...
        Catch ex As Exception
            Response.Write(DirectCast(ex.Message() & "_exec Coll_XtraReport_Transaksi '" & Session("UserName") & "','" & Format(dt_strdate.Value, "yyyy-MM-dd") & "','" & Format(dt_endate.Value, "yyyy-MM-dd") & "'", String))
        End Try
        ASPxGridView1.DataBind()
    End Sub
    Private Sub ASPxGridView1_Init(sender As Object, e As EventArgs) Handles ASPxGridView1.Init
        XtraHistoryTransaksi.SelectCommand = "SELECT ROW_NUMBER() OVER(ORDER BY ID DESC) AS NoUrut, * FROM Coll_TrxReport WHERE UserName='" & Session("UserName") & "'"
    End Sub
