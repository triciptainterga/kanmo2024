...
function getDataPrimary() {
    ...
    document.getElementById("iframe_wb").src = "WB/main/Tele_Dashboard.html?UserName=" + UserName + "&NamaUserLogin=" + NamaUserLogin;
}
...
function summaryCall() {
    $.ajax({
        type: "POST",
        url: "../../asmx/Tele_Dashboard.asmx/Tele_DashboardSummaryCall",
		//url: "https://cloud.uidesk.id/v2uidesk/apps/asmx/Tele_Dashboard.asmx/Tele_DashboardSummaryCall?UserName=Admin&XStartDate=2023-01-01&XEndDate=2023-09-30",
		//data: {},
        //data: JSON.stringify({ UserName: 'Admin', XStartDate: $('#start_date').val(), XEndDate: $('#end_date').val() }),
        data: JSON.stringify({ UserName: 'Admin', XStartDate: '2023-01-01', XEndDate: '2023-10-30' }),
...
function durationCall() {
    $.ajax({
        type: "POST",
        url: "../../asmx/Tele_Dashboard.asmx/Tele_DashboardDurationCall",
        data: JSON.stringify({ UserName: 'Admin', XStartDate: $('#start_date').val(), XEndDate: $('#end_date').val() }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (result) {
...
function summaryData() {
    ...
    $.ajax({
        type: "POST",
        url: "../../asmx/Tele_Dashboard.asmx/Tele_DashboardSummaryData",
        // data: JSON.stringify({ UserName: 'Admin', XStartDate: $('#start_date').val(), XEndDate: $('#end_date').val() }),
        data: JSON.stringify({ UserName: 'Admin', XStartDate: '2023-01-01', XEndDate: '2023-09-30' }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (result) {
            let datas = JSON.parse(result.d);
            console.log("datas", datas)
            $('#Anser').text(datas[0].Anser)
            $('#Reject').text(datas[0].Reject)
            $('#NoAnser').text(datas[0].NoAnser)
            $('#Busy').text(datas[0].Busy)
            $('#WrongNumber').text(datas[0].WrongNumber)
            $('#Unregistered').text(datas[0].Unregistered)
