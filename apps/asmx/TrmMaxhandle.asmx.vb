...
Public Class TrmMaxhandle1
    ...
    'Dim strLogTime As String = HttpContext.Current.Session("UserName").ToString
    ...
    Public Function TransactionMaxHandle(ByVal TrxID As String, ByVal TrxMaxHandle As String, ByVal TrxUserName As String, ByVal TrxAction As String) As String
        Dim listTickets As List(Of resultInsert) = New List(Of resultInsert)()
        Dim strExec As String = String.Empty

        Dim TrxMaxHandleXSS As String = AntiXssEncoder.HtmlEncode(TrxMaxHandle.Trim, True)
        ...
        Try
            Using con As New SqlConnection(constr)
                ...
                sqlComm.CommandText = "UIDESK_TrxMaxHandle"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("TrxID", TrxID)
                sqlComm.Parameters.AddWithValue("TrxMaxHandle", TrxMaxHandleXSS)
        ...
        Catch ex As Exception
            ...
            strExec = "exec UIDESK_TrxMaxHandle '" & TrxID & "','" & TrxMaxHandleXSS & "','" & TrxUserNameXSS & "','" & TrxAction & "'"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function TableMaxHandle(ByVal TrxUserName As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim TrxAction As String = "TABLE"
        Dim NameSP = "Exec UIDESK_TrxMaxHandle"
        Dim ExecSP = "" & NameSP & " '0','0','" & TrxUserName & "','" & TrxAction & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_TrxMaxHandle", conn)
                sqlComm.Parameters.AddWithValue("@TrxID", "0")
                sqlComm.Parameters.AddWithValue("@TrxMaxHandle", "0")
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function SelectMaxHandle(ByVal TrxID As String, ByVal TrxUserName As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()
