...
Public Class TrmStatusCall1
    ...
    Public Function InsertStatusCall(ByVal status_id As String, ByVal status_call As String, ByVal TrxUserName As String)
        Dim listTickets As List(Of resultInsert) = New List(Of resultInsert)()
        Dim status_callXSS As String = AntiXssEncoder.HtmlEncode(status_call.Trim, True)
        Dim TrxUserNameXSS As String = AntiXssEncoder.HtmlEncode(TrxUserName.Trim, True)
        Dim strExec As String = String.Empty
        Dim status_action As String = "Insert"
        Dim constr As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Try
            Using con As New SqlConnection(constr)
                Dim sqlComm As New SqlCommand()
                sqlComm.Connection = con
                sqlComm.CommandText = "UIDESK_TrxStatusCall"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("status_id", status_id)
                sqlComm.Parameters.AddWithValue("status_call", status_callXSS)
                sqlComm.Parameters.AddWithValue("status_username", TrxUserNameXSS)
                sqlComm.Parameters.AddWithValue("status_action", status_action)
                con.Open()
        ...
        Catch ex As Exception
            ...
            strExec = "exec UIDESK_TrxStatusCall '" & status_id & "','" & status_callXSS & "','" & TrxUserNameXSS & "','" & status_action & "'"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function UpdateStatusCall(ByVal status_id As String, ByVal status_call As String, ByVal TrxUserName As String)
        Dim listTickets As List(Of resultInsert) = New List(Of resultInsert)()
        Dim status_callXSS As String = AntiXssEncoder.HtmlEncode(status_call.Trim, True)
        Dim TrxUserNameXSS As String = AntiXssEncoder.HtmlEncode(TrxUserName.Trim, True)
        Dim strExec As String = String.Empty
        Dim status_action As String = "Update"
        Dim constr As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Try
            Using con As New SqlConnection(constr)
                Dim sqlComm As New SqlCommand()
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function DeleteStatusCall(ByVal status_id As String, ByVal status_call As String, ByVal TrxUserName As String)
        Dim listTickets As List(Of resultInsert) = New List(Of resultInsert)()
        Dim status_callXSS As String = AntiXssEncoder.HtmlEncode(status_call.Trim, True)
        Dim TrxUserNameXSS As String = AntiXssEncoder.HtmlEncode(TrxUserName.Trim, True)
        Dim strExec As String = String.Empty
        Dim status_action As String = "Delete"
        Dim constr As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Try
            Using con As New SqlConnection(constr)
                Dim sqlComm As New SqlCommand()
