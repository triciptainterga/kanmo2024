...
<ScriptService()>
<ToolboxItem(False)>
Public Class TrxHistoryOutbound1
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function UIDESK_TrxHistoryOutbound(ByVal TrxStartDate As String, ByVal TrxEnddate As String, ByVal TrxUserName As String, ByVal TrxPolisNumber As String, ByVal TrxPhoneNumber As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec UIDESK_TrxHistoryOutbound"
        Dim ExecSP = "" & NameSP & " '" & TrxStartDate & "','" & TrxEnddate & "','" & TrxUserName & "','" & TrxPolisNumber & "','" & TrxPhoneNumber & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_TrxHistoryOutbound", conn)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function CheckSummaryDay(ByVal StartDate As String, ByVal EndDate As String, ByVal TrxUserName As String, ByVal TrxAction As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim NameSP = "Exec UIDESK_TrmCheckSummaryDay"
        Dim ExecSP = "" & NameSP & " '" & StartDate & "','" & EndDate & "','" & TrxUserName & "','" & TrxAction & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_TrmCheckSummaryDay", conn)
