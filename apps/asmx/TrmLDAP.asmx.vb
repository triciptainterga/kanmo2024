...
Public Class TrmLDAP1
    ...
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function UpdateConfigurationLDAP(ByVal TrxUsername As String, ByVal TrxLDAPServer As String, ByVal TrxLDAPUser As String,
                                                 ByVal TrxLDAPPassword As String, ByVal TrxLDAPStatus As String, ByVal TrxID As String) As String
        Dim listTickets As List(Of ResultInsert) = New List(Of ResultInsert)()
        Dim TrxLDAPServerXSS As String = AntiXssEncoder.HtmlEncode(TrxLDAPServer.Trim, True)
        Dim TrxLDAPUserXSS As String = AntiXssEncoder.HtmlEncode(TrxLDAPUser.Trim, True)
        Dim TrxLDAPPasswordXSS As String = AntiXssEncoder.HtmlEncode(TrxLDAPPassword.Trim, True)
        Dim TrxLDAPStatusXSS As String = AntiXssEncoder.HtmlEncode(TrxLDAPStatus.Trim, True)
        ...
        Try
            Using con As New SqlConnection(constr)
                ...
                sqlComm.CommandText = "UIDESK_TrxLDAP"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("TrxLDAPServer", TrxLDAPServerXSS)
                sqlComm.Parameters.AddWithValue("TrxLDAPUser", TrxLDAPUserXSS)
                sqlComm.Parameters.AddWithValue("TrxLDAPPassword", TrxLDAPPasswordXSS)
                sqlComm.Parameters.AddWithValue("TrxLDAPStatus", TrxLDAPStatusXSS)
        ...
        Catch ex As Exception
            ...
            strExec = "exec UIDESK_TrxLDAP " & "'" & TrxUserNameXSS & "'," & "'" & TrxLDAPServerXSS & "'," & "'" & TrxLDAPUserXSS & "'," & "'" & TrxLDAPPasswordXSS & "'," & "'" & TrxLDAPStatusXSS & "'," & "'" & TrxID & "'"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function ConnectConfigurationLDAP(ByVal TrxLDAPServer As String, ByVal TrxLDAPUser As String, ByVal TrxLDAPPassword As String) As String
        Dim Success As Boolean = False
        Dim Entry As New System.DirectoryServices.DirectoryEntry("LDAP://" & TrxLDAPServer & ":389", TrxLDAPUser, TrxLDAPPassword)
        Dim Searcher As New System.DirectoryServices.DirectorySearcher(Entry)
        Searcher.SearchScope = DirectoryServices.SearchScope.OneLevel
        Try
            Dim Results As System.DirectoryServices.SearchResult = Searcher.FindOne
            Success = Not (Results Is Nothing)
            LogSuccess(HttpContext.Current.Session("UserName"), "LDAP Success - " & TrxLDAPUser & "")
        Catch ex As Exception
            Success = False
            LogError(HttpContext.Current.Session("UserName"), ex, "LDAP Failed - " & TrxLDAPUser & "")
        End Try
        Return Success
    End Function
End Class