...
<ScriptService()>
<ToolboxItem(False)>
Public Class TrmTemplateAutoReply1
    ...
    Public Function AHU_Uidesk_TrxEmailAutoReply_TSIUD(ByVal ID As String, ByVal Template As String, ByVal Account As String,
                                                 ByVal State As String, ByVal User As String, ByVal Action As String) As String
        If Action = "TABLE" Or Action = "SELECT" Then
            ...
            Dim NameSP = "Exec AHU_Uidesk_TrxEmailAutoReply"
            Dim ExecSP = "" & NameSP & " '0','" & ID & "','" & Template & "','" & Account & "','" & State & "','" & User & "','" & Action & "'"
            Try
                Using conn As SqlConnection = New SqlConnection(connstring)
                    conn.Open()
                    Dim sqlComm As SqlCommand = New SqlCommand("AHU_Uidesk_TrxEmailAutoReply", conn)
                    sqlComm.Parameters.AddWithValue("@Modul", 0)
                    sqlComm.Parameters.AddWithValue("@ID", ID)
                    sqlComm.Parameters.AddWithValue("@Template", Template)
                    sqlComm.Parameters.AddWithValue("@Account", Account)
                    sqlComm.Parameters.AddWithValue("@State", Action)
        ...
        Else
            ...
            Try
                Using con As New SqlConnection(constr)
                    ...
                    sqlComm.CommandText = "AHU_Uidesk_TrxEmailAutoReply"
                    sqlComm.CommandType = CommandType.StoredProcedure
                    sqlComm.Parameters.AddWithValue("Modul", 0)
                    sqlComm.Parameters.AddWithValue("ID", ID)
                    sqlComm.Parameters.AddWithValue("Template", Template)
            ...
            Catch ex As Exception
                ...
                strExec = "exec AHU_Uidesk_TrxEmailAutoReply '" & ID & "','" & Template & "','" & Account & "','" & State & "','" & User & "','" & Action & "'"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function AHU_Uidesk_TrxEmailAutoReply_Setting(ByVal ID As String, ByVal Template As String, ByVal Account As String,
                                                ByVal State As String, ByVal User As String, ByVal Action As String) As String
        ...
        Dim NameSP = "Exec AHU_Uidesk_TrxEmailAutoReply"
        Dim ExecSP = "" & NameSP & " '" & ID & "','" & Template & "','" & Account & "','" & State & "','" & User & "','" & Action & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("AHU_Uidesk_TrxEmailAutoReply", conn)
                sqlComm.Parameters.AddWithValue("@Modul", 1)
                sqlComm.Parameters.AddWithValue("@ID", ID)
                sqlComm.Parameters.AddWithValue("@Template", Template)
                sqlComm.Parameters.AddWithValue("@Account", Account)
                sqlComm.Parameters.AddWithValue("@State", Action)
