...
Public Class TrxOutboundCall1
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function InsertTransactionOutboundCall(ByVal TrxUserName As String, ByVal TrxCallid As String, ByVal TrxPhoneNumber As String, ByVal TrxReasonCode As String,
                                                  ByVal TrxCallStatus As String, ByVal TrxEscalationTicket As String, ByVal TrxResponse As String) As String
        ...
        Try
            Using con As New SqlConnection(constr)
                ...
                sqlComm.CommandText = "UIDESK_TrxTransactionOutboundCall"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("TrxUserName", TrxUserName)
                sqlComm.Parameters.AddWithValue("TrxCallid", TrxCallid)
                sqlComm.Parameters.AddWithValue("TrxPhoneNumber", TrxPhoneNumber)
                sqlComm.Parameters.AddWithValue("TrxReasonCode", TrxReasonCode)
                sqlComm.Parameters.AddWithValue("TrxCallStatus", TrxCallStatus)
                sqlComm.Parameters.AddWithValue("TrxEscalationTicket", TrxEscalationTicket)
        ...
        Catch ex As Exception
            ...
            strExec = "exec UIDESK_TrxTransactionOutboundCall '" & TrxUserName & "','" & TrxCallid & "','" & TrxPhoneNumber & "','" & TrxReasonCode & "','" & TrxCallStatus & "','" & TrxEscalationTicket & "','" & HttpUtility.UrlDecode(TrxResponse) & "'"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function InsertTransactionReminderOutbound(ByVal TrxUserName As String, ByVal TrxCallid As String, ByVal TrxPhoneNumber As String,
                                                      ByVal TrxReminderSubject As String, ByVal TrxReminderStatus As String, ByVal TrxReminderDate As String,
                                                      ByVal TrxReminderDescription As String) As String
        Dim listTickets As List(Of resultInsert) = New List(Of resultInsert)()
        Dim strExec As String = String.Empty
        Dim _CustomerID As String = String.Empty
        Dim _Result As String = String.Empty
        ...
        Try
            Using con As New SqlConnection(constr)
                Dim sqlComm As New SqlCommand()
                sqlComm.Connection = con
                sqlComm.CommandText = "UIDESK_TrxTransactionReminderCall"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("TrxUserName", TrxUserName)
                sqlComm.Parameters.AddWithValue("TrxCallid", TrxCallid)
                sqlComm.Parameters.AddWithValue("TrxPhoneNumber", TrxPhoneNumber)
                sqlComm.Parameters.AddWithValue("TrxReminderSubject", TrxReminderSubject)
                sqlComm.Parameters.AddWithValue("TrxReminderStatus", TrxReminderStatus)
                sqlComm.Parameters.AddWithValue("TrxReminderDate", TrxReminderDate)
                sqlComm.Parameters.AddWithValue("TrxReminderDescription", HttpUtility.UrlDecode(TrxReminderDescription))
                con.Open()
                sqlComm.ExecuteNonQuery()
                con.Close()
            End Using
        Catch ex As Exception
            ...
            objectTickets.msgSystem = ex.Message()
            listTickets.Add(objectTickets)
            strExec = "exec UIDESK_TrxTransactionReminderCall '" & TrxUserName & "','" & TrxCallid & "','" & TrxPhoneNumber & "','" & TrxReminderSubject & "','" & TrxReminderStatus & "','" & TrxReminderDate & "','" & HttpUtility.UrlDecode(TrxReminderDescription) & "'"
            LogError(HttpContext.Current.Session("UserName"), ex, strExec)
        Finally
            Dim objectTickets As resultInsert = New resultInsert()
            objectTickets.Result = "True"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function TransactionOutboundHeader(ByVal TrxID As String, ByVal TrxPhoneNumber As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()
        Dim sql As String = ""
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                ...
                sql = "exec UIDESK_TrxViewOutboundHeader '" & TrxID & "','" & TrxPhoneNumber & "'"
