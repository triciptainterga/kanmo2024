...
<ScriptService()>
<ToolboxItem(False)>
Public Class Out_TrxTaskboard1
    ...
    Public Function TaskboardCounting(ByVal TrxUserName As String) As String
        ...
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                ...
                sql = "exec Out_TrxTaskboardCountingCall '" & TrxUserName & "'"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function OutTaskboard(ByVal TrxID As String, ByVal TrxUserName As String, ByVal TrxLevelUser As String) As String
        ...
        Try
            strQuery = "Exec Out_Taskboard '" & TrxID & "', '" & TrxUserName & "', '" & TrxLevelUser & "'"
            Using con As SqlConnection = New SqlConnection(cs)
                ...
                While rdr.Read()
                    ...
                    objectTickets.TrxCounting = rdr("CountingCall").ToString()
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function OutTaskboardPending(ByVal UserName As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()
        Dim sql As String = ""
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                ...
                sql = "exec Out_TaskboardPending '" & UserName & "'"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function SimpanTransaksi(ByVal HeaderID As String, ByVal Telepon As String, ByVal Keterangan As String, ByVal StatusTelepon As String,
                                    ByVal StatusData As String, ByVal Tanggal As String, ByVal UserName As String) As String
        Dim listTickets As List(Of resultInsert) = New List(Of resultInsert)()
        Dim strExec As String = String.Empty
        Dim TeleponXSS As String = AntiXssEncoder.HtmlEncode(Telepon.Trim, True)
        Dim constr As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Try
            Using con As New SqlConnection(constr)
                Dim sqlComm As New SqlCommand()
                sqlComm.Connection = con
                sqlComm.CommandText = "Out_SimpanTransaksi"
                sqlComm.CommandType = CommandType.StoredProcedure
                sqlComm.Parameters.AddWithValue("HeaderID", HeaderID)
                sqlComm.Parameters.AddWithValue("Telepon", TeleponXSS)
                sqlComm.Parameters.AddWithValue("Keterangan", Keterangan)
                sqlComm.Parameters.AddWithValue("StatusTelepon", StatusTelepon)
                sqlComm.Parameters.AddWithValue("StatusData", StatusData)
                sqlComm.Parameters.AddWithValue("Tanggal", Tanggal)
        ...
        Catch ex As Exception
            ...
            listTickets.Add(objectTickets)
            strExec = "exec Out_SimpanTransaksi '" & HeaderID & "','" & TeleponXSS & "','" & Keterangan & "','" & StatusTelepon & "','" & StatusData & "','" & Tanggal & "','" & UserName & "'"
            LogError(HttpContext.Current.Session("UserName"), ex, strExec)
        Finally
            Dim objectTickets As resultInsert = New resultInsert()
            objectTickets.Result = "True"
            objectTickets.msgSystem = "Data Has Been Send"
            listTickets.Add(objectTickets)
            strExec = "exec Out_SimpanTransaksi '" & HeaderID & "','" & TeleponXSS & "','" & Keterangan & "','" & StatusTelepon & "','" & StatusData & "','" & Tanggal & "','" & UserName & "'"
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function SimpanEPIC(ByVal UserName As String, ByVal Telepon As String, ByVal Action As String) As String
        Dim listTickets As List(Of resultInsert) = New List(Of resultInsert)()
        Dim strExec As String = String.Empty
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()
        ...
        If Action = "SELECT" Then
            Try
                Using conn As SqlConnection = New SqlConnection(connstring)
                    conn.Open()
                    sql = "exec KAN_Uidesk_EpicOutboundCall '" & UserName & "','" & Telepon & "','" & Action & "'"
                    Dim ad As SqlDataAdapter = New SqlDataAdapter(sql, conn)
                    Dim ds As DataSet = New DataSet()
                    ad.Fill(ds)
                    dt = ds.Tables(0)
        ...
        Else
            Try
                Using con As New SqlConnection(constr)
                    Dim sqlComm As New SqlCommand()
                    sqlComm.Connection = con
                    sqlComm.CommandText = "KAN_Uidesk_EpicOutboundCall"
                    sqlComm.CommandType = CommandType.StoredProcedure
                    sqlComm.Parameters.AddWithValue("UserName", UserName)
            ...
            Catch ex As Exception
                ...
                objectTickets.msgSystem = ex.Message()
                listTickets.Add(objectTickets)
                strExec = "exec KAN_Uidesk_EpicOutboundCall '" & UserName & "','" & Telepon & "','" & Action & "'"
                LogError(HttpContext.Current.Session("UserName"), ex, strExec)
            Finally
                Dim objectTickets As resultInsert = New resultInsert()
                objectTickets.Result = "True"
