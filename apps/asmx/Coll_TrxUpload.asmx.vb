...
Public Class Coll_TrxUpload1
    ...
    Dim FileCollection As String = "~/FileCollection/"
    ...
    Public Function CollUpload_1() As Response
        ...
        Dim SavePath As String = HttpContext.Current.Server.MapPath("~/FileCollection/")
        ...
        Dim DirectoryX As String = Path.Combine(Server.MapPath(FileCollection & "" & strTime.Replace("..", "")))
        If Not System.IO.Directory.Exists(DirectoryX) Then
            System.IO.Directory.CreateDirectory(DirectoryX)
        End If
        For i As Integer = 0 To Files.Count - 1
            ...
            For j As Integer = 0 To validFileTypes.Length - 1
                If FileExt = "." & validFileTypes(j) Then
                    ...
                    Dim FilePath As String = FileCollection & "" & strTime.Replace("..", "") & "/"
        ...
        Try
            ...
            Dim json As String = "{""asal"":""FileCollection/" & strTime & "/" & String.Concat(FileId, FileExt) & """,""tujuan"":""FileCollection/" & strTime & "/" & String.Concat(FileId, FileExt) & """, ""app"":""UIDESK_V2""}"
        ...
        Try
            Using con As New SqlConnection(constr)
                ...
                sqlComm.CommandText = "Coll_Collection_Excel"
            ...
            End Using
        Catch ex As Exception
            stringNya = "exec Coll_Collection_Excel '" & strTime & "','" & String.Concat(FileId) & "','" & uploadby & "','" & ProdukID & "'"
        ...
        'Dim Name, Email, Telepon, BirthDate, Gender, JobTitle, Status, Address,
        'TypePinjaman, NamaPinjaman, TotalPinjaman, JumlahTagihan, PeriodeFrom, PeriodeTo, PeriodePinjaman, JatuhTempo As String

        'Try
        '    Dim path As String = HttpContext.Current.Server.MapPath("~/FileCollection/" & strTime.Replace("..", "") & "/" & String.Concat(FileId, FileExt))
        ...
        '    Dim sqlImport As String = "exec Coll_Collection_Excel '" & strTime & "','" & String.Concat(FileId) & "'"
        ...
        '        TypePinjaman = rdr("F9").ToString
        '        NamaPinjaman = rdr("F10").ToString
        '        TotalPinjaman = rdr("F11").ToString
        '        JumlahTagihan = rdr("F12").ToString
        '        PeriodeFrom = rdr("F13").ToString
        '        PeriodeTo = rdr("F14").ToString
        '        PeriodePinjaman = rdr("F15").ToString
        '        JatuhTempo = rdr("F16").ToString

        '        Try
        '            Using con As New SqlConnection(constr)
        '                Dim sqlComm As New SqlCommand()
        '                sqlComm.Connection = con
        '                sqlComm.CommandText = "Coll_Collection_Upload"
        '                sqlComm.CommandType = CommandType.StoredProcedure
        ...
        '                sqlComm.Parameters.AddWithValue("TypePinjaman", TypePinjaman)
        '                sqlComm.Parameters.AddWithValue("NamaPinjaman", NamaPinjaman)
        '                sqlComm.Parameters.AddWithValue("TotalPinjaman", TotalPinjaman)
        '                sqlComm.Parameters.AddWithValue("JumlahTagihan", JumlahTagihan)
        '                sqlComm.Parameters.AddWithValue("PeriodeFrom", PeriodeFrom)
        '                sqlComm.Parameters.AddWithValue("PeriodeTo", PeriodeTo)
        '                sqlComm.Parameters.AddWithValue("PeriodePinjaman", PeriodePinjaman)
        ...
        '            End Using
        '        Catch ex As Exception
        '            strExec = "exec Coll_Collection_Upload '" & Name & "','" & Email & "','" & Telepon & "','" & BirthDate & "','" & Gender & "','" & JobTitle & "','" & Status & "','" & Address & "','" & TypePinjaman & "','" & NamaPinjaman & "','" & TotalPinjaman & "','" & JumlahTagihan & "','" & PeriodeFrom & "','" & PeriodeTo & "','" & PeriodePinjaman & "','" & JatuhTempo & "','" & ProdukID & "','" & strTime & "','" & uploadby & "'"
    ...
    Public Function DistributeData(ByVal TrxUploadID As String, ByVal TrxUserName As String, ByVal TrxAction As String) As String
        ...
        Try
            Using con As New SqlConnection(constr)
                Dim sqlComm As New SqlCommand()
                sqlComm.Connection = con
                sqlComm.CommandText = "Coll_DistibuteData"
                sqlComm.CommandType = CommandType.StoredProcedure
        ...
        Catch ex As Exception
            ...
            strExec = "exec Coll_DistibuteData '" & TrxUploadID & "','" & TrxUserName & "','" & TrxAction & "'"
            LogError(HttpContext.Current.Session("UserName"), ex, strExec)
        Finally
            Dim objectTickets As resultInsert = New resultInsert()
