...
<ScriptService()>
<ToolboxItem(False)>
Public Class Coll_TrxTaskboard1
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function CollTaskboard(ByVal TrxID As String, ByVal TrxUserName As String, ByVal TrxLevelUser As String) As String
        ...
        Try
            strQuery = "Exec Coll_Taskboard '" & TrxID & "', '" & TrxUserName & "', '" & TrxLevelUser & "'"
            Using con As SqlConnection = New SqlConnection(cs)
                ...
                While rdr.Read()
                    ...
                    objectTickets.TrxTotalPinjaman = rdr("TotalPinjaman").ToString()
                    objectTickets.TrxJumlahTagihan = rdr("JumlahTagihan").ToString()
                    objectTickets.TrxJatuhTempo = rdr("JatuhTempo").ToString()
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function CollTaskboardPending(ByVal UserName As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()
        Dim sql As String = ""
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                ...
                sql = "exec Coll_TaskboardPending '" & UserName & "'"
