...
Public Class TrxHistoryEmail1
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function UIDESK_TrxHistoryEmail(ByVal TrxID As String, ByVal TrxStartDate As String, ByVal TrxEnddate As String, ByVal TrxUserName As String, ByVal TrxAddress As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()
        ...
        If TrxAddress = "" Then
            EmailAddress = "-"
        Else
            EmailAddress = TrxAddress
        End If
        Dim NameSP = "Exec UIDESK_TrxHistoryEmail"
        Dim ExecSP = "" & NameSP & " '" & TrxID & "','" & TrxStartDate & "','" & TrxEnddate & "','" & TrxUserName & "','" & TrxAddress & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_TrxHistoryEmail", conn)
                ...
                sqlComm.Parameters.AddWithValue("@TrxAddress", EmailAddress)
