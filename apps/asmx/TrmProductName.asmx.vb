...
Public Class TrmProductName1
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function Uidesk_MBrandName(ByVal TrxUserName As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim TrxAction As String = "Uidesk_MBrandName"
        Dim NameSP = "Exec UIDESK_TrxProductNameBL"
        Dim ExecSP = "" & NameSP & " '0','" & TrxUserName & "','0','0','0','" & TrxAction & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function BrandCategory(ByVal TrxID As String, ByVal TrxUserName As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim TrxAction As String = "BrandCategory"
        Dim NameSP = "Exec UIDESK_TrxProductNameBL"
        Dim ExecSP = "" & NameSP & " '" & TrxID & "','" & TrxUserName & "','0','0','0','" & TrxAction & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
