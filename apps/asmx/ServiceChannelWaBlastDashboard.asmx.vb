...
<ScriptService()>
<ToolboxItem(False)>
Public Class ServiceChannelWaBlastDashboard
    Inherits System.Web.Services.WebService

    Dim sqlcom As SqlCommand
    Dim sqlcon As New SqlConnection(ConfigurationManager.ConnectionStrings("WAConnection").ConnectionString)
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function UpdateDataBlast(ByVal Keterangan As String, ByVal idTable As String) As String
        Dim listTickets As List(Of listTransactionThread) = New List(Of listTransactionThread)()
        Dim cs As String = ConfigurationManager.ConnectionStrings("WAConnection").ConnectionString
        Dim strQuery As String = String.Empty
        Dim LevelUser As String = String.Empty
        Try
            strQuery = "Exec WA_Update_TableToBlast '" & Keterangan & "','" & idTable & "'"
            Using con As SqlConnection = New SqlConnection(cs)
                Dim cmd As SqlCommand
                cmd = New SqlCommand(strQuery, con)
                cmd.CommandType = CommandType.Text
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function ws_3_ChannelWaBlastDashboard(ByVal UserID As String, ByVal LayerID As String, ByVal Spv As String) As String
