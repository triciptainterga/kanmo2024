...
<ScriptService()>
<ToolboxItem(False)>
Public Class TrxHistoryTicket1
    ...
    Public Function UIDESK_TrxHistoryTicket(ByVal TrxStartDate As String, ByVal TrxEnddate As String, ByVal TrxUserName As String, ByVal TrxTicketNumber As String) As String
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()

        Dim TrxTicketNumberValue As String = String.Empty
        If TrxTicketNumber = "" Then
            TrxTicketNumberValue = "-"
        Else
            TrxTicketNumberValue = TrxTicketNumber
        End If
        Dim NameSP = "Exec UIDESK_TrxHistoryTicket"
        Dim ExecSP = "" & NameSP & " '" & TrxStartDate & "','" & TrxEnddate & "','" & TrxUserName & "','" & TrxTicketNumber & "'"
        Try
            Using conn As SqlConnection = New SqlConnection(connstring)
                conn.Open()
                Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_TrxHistoryTicket", conn)
                ...
                sqlComm.Parameters.AddWithValue("@TrxTicketNumber", TrxTicketNumberValue)
    ...
    '<WebMethod(EnableSession:=True)>
    '<ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    'Public Function CheckSummaryDay(ByVal StartDate As String, ByVal EndDate As String, ByVal TrxUserName As String) As String
