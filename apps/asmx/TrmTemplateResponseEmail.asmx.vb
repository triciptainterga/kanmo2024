...
<ScriptService()>
<ToolboxItem(False)>
Public Class TrmTemplateResponseEmail1
    ...
    Public Function AHU_Uidesk_TrxFormatResponseEmail_TSIUD(ByVal ID As String, ByVal Template As String, ByVal Format As String,
        ...
        If Action = "TABLE" Or Action = "SELECT" Then
            ...
            Dim NameSP = "Exec AHU_Uidesk_TrxFormatResponseEmail"
            Dim ExecSP = "" & NameSP & " '" & ID & "','" & Template & "','" & Format & "','" & State & "','" & User & "','" & Action & "'"
            Try
                Using conn As SqlConnection = New SqlConnection(connstring)
                    conn.Open()
                    Dim sqlComm As SqlCommand = New SqlCommand("AHU_Uidesk_TrxFormatResponseEmail", conn)
                    sqlComm.Parameters.AddWithValue("@ID", ID)
                    sqlComm.Parameters.AddWithValue("@Template", Template)
                    sqlComm.Parameters.AddWithValue("@Format", Format)
        ...
        Else
            ...
            Try
                Using con As New SqlConnection(constr)
                    ...
                    sqlComm.CommandText = "AHU_Uidesk_TrxFormatResponseEmail"
                    sqlComm.CommandType = CommandType.StoredProcedure
                    sqlComm.Parameters.AddWithValue("ID", ID)
                    sqlComm.Parameters.AddWithValue("Template", Template)
                    sqlComm.Parameters.AddWithValue("Format", Format)
            ...
            Catch ex As Exception
                ...
                strExec = "exec AHU_Uidesk_TrxFormatResponseEmail '" & ID & "','" & Template & "','" & Format & "','" & State & "','" & User & "','" & Action & "'"
