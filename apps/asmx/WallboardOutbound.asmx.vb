...
<ScriptService()>
<ToolboxItem(False)>
Public Class WallboardOutbound
    ...
    <WebMethod()>
    <ScriptMethod(UseHttpGet:=False, ResponseFormat:=ResponseFormat.Json)>
    Public Function listDataOutbound(ByVal JenisWB As String) As String
        'Dim listTickets As List(Of dataStatusCall) = New List(Of dataStatusCall)()
        'Dim strExec As String = String.Empty
        'Dim _TrxSubject As String = String.Empty
        'Dim _TrxNameNya As String = String.Empty
        'Dim _TrxTotalNya As String = String.Empty
        ...
        '        sqlComm.CommandText = "WB_Out_Data"
        '        sqlComm.CommandType = CommandType.StoredProcedure
        '        sqlComm.Parameters.AddWithValue("@JenisWB", JenisWB)

        '        con.Open()
        '        sqldr = sqlComm.ExecuteReader()
        '        While sqldr.Read()
        '            _TrxNameNya &= sqldr("NameNya").ToString
        '            _TrxTotalNya &= sqldr("TotalNya").ToString
        ...
        '    Dim objectTickets As dataStatusCall = New dataStatusCall()
        '    objectTickets.Result = "False"
        '    objectTickets.TrxmsgSystem = ex.Message()
        '    listTickets.Add(objectTickets)
        '    strExec = "exec WB_Out_Data '" & JenisWB & "'"
        '    LogError(strLogTime, ex, strExec)
        'Finally
        '    Dim objectTickets As dataStatusCall = New dataStatusCall()
        '    If _Result = "Success" Then
        '        objectTickets.Result = "True"
        '        objectTickets.NameNya = _TrxNameNya
        '        objectTickets.TotalNya = _TrxTotalNya
        '        objectTickets.TrxmsgSystem = "Data Tersedia dan siap dikonsumsi"
        '        listTickets.Add(objectTickets)
        '        strExec = "exec WB_Out_Data '" & JenisWB & "'"
        ...
        '        objectTickets.NameNya = _TrxNameNya
        '        objectTickets.TotalNya = _TrxTotalNya
        '        objectTickets.TrxmsgSystem = "Data tidak tersedia"
        '        listTickets.Add(objectTickets)
        '        strExec = "exec WB_Out_Data '" & JenisWB & "'"
        '        LogSuccess(strLogTime, strExec)
        '    End If
    ...
    Public Sub LogSuccess(ByVal agentName As String, strValue As String)
        ...
        Try
            Dim DirectoryX As String = Path.Combine(Server.MapPath("~/apps/ErrorLog/WallboardOutbound/" & DateTime.Now.ToString("ddMMyyyy")))
        ...
        Catch exX As Exception
            ...
            Dim pathXX As String = HttpContext.Current.Server.MapPath("~/apps/ErrorLog/WallboardOutbound/" & DateTime.Now.ToString("ddMMyyyy") & ".txt")
            ...
            End Using
        Finally
            Dim pathX As String = HttpContext.Current.Server.MapPath("~/apps/ErrorLog/WallboardOutbound/" & DateTime.Now.ToString("ddMMyyyy") & "/" & DateTime.Now.ToString("ddMMyyyy") & ".txt")
