...
Public Class BantuDagang
    ...
    Public Function DataAccessToken(ByVal UserName As String, ByVal Token As String, ByVal Action As String) As String
        ...
        Dim _ResultToken As String = String.Empty
        Dim _OrderID As String = String.Empty
        Dim connstring As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Dim dt As DataTable = New DataTable()
        If Action = "Select" Then
            Dim NameSP = "Exec UIDESK_TrxToken"
            Dim ExecSP = "" & NameSP & " '-','" & UserName & "','" & Token & "','" & Action & "'"
            Try
                Using conn As SqlConnection = New SqlConnection(connstring)
                    conn.Open()
                    Dim sqlComm As SqlCommand = New SqlCommand("UIDESK_TrxToken", conn)
        ...
        Else
            Try
                Using con As New SqlConnection(constr)
                    ...
                    sqlComm.CommandText = "UIDESK_TrxToken"
                    sqlComm.CommandType = CommandType.StoredProcedure
                    sqlComm.Parameters.AddWithValue("UserName", UserName)
                    sqlComm.Parameters.AddWithValue("Token", Token)
                    sqlComm.Parameters.AddWithValue("Action", Action)
                    ...
                    While sqldr.Read()
                        _ResultToken &= sqldr("Token").ToString
                        _OrderID &= sqldr("OrderID").ToString
                    End While
                    sqldr.Close()
                    con.Close()
                End Using
            Catch ex As Exception
                ...
                strExec = "exec UIDESK_TrxToken '" & UserName & "','" & Token & "','" & Action & "'"
                LogError(HttpContext.Current.Session("UserName"), ex, strExec)
            Finally
                Dim objectTickets As resultInsert = New resultInsert()
                objectTickets.Result = "True"
                objectTickets.TrxmsgSystem = _ResultToken
                objectTickets.TrxID = _OrderID
                listTickets.Add(objectTickets)
                strExec = "exec UIDESK_TrxToken '" & UserName & "','" & Token & "','" & Action & "'"
